  ...
  ProductApp.controller('productController', function($scope,$http,$window,$rootScope) {
	  ...
	  $scope.userddetils = {};
			...
			$scope.custid = $window.localStorage["custid"];
			$scope.custid = $scope.custid.replace(/\"/g,""); 
			$scope.cartlength = 0;
			$scope.notificationcount = 0;
			
			if($scope.userlevel == 'Superadmin')
			{
				$scope.Userlevelsarray = ['Admin','Customer']
			}
			if($scope.userlevel == 'Admin')
			{
				$scope.Userlevelsarray = ['Admin','Customer']
			}
			if($scope.userlevel == 'Customer')
			{
				$scope.Userlevelsarray = ['Customer']
			}
			...
			$scope.ListUsers = function()
			{
				...
				$http({
              method: 'GET'
              , url: 'http://103.252.7.5:8086/api/ListUsers/'+localStorage["companyid"]+'/'+$scope.userlevel+'/'+$scope.userid+'/'+localStorage["custid"]
              , dataType: 'jsonp'
			}).then(function (response) {
				...
				/* $scope.ListOrders = function()
			{
				...
				$http({
              method: 'GET'
              , url: 'http://103.252.7.5:8086/api/ListOrders/'+localStorage["companyid"]+'/'+$scope.userlevel+'/'+$scope.userid+'/'+localStorage["custid"]
              , dataType: 'jsonp'
			}).then(function (response) {
			...
			$scope.OrderListonMonth = function(month)
			{
				...
				$http({
              method: 'GET'
              , url: 'http://103.252.7.5:8086/api/OrderListonMonth/'+localStorage["companyid"]+'/'+$scope.userlevel+'/'+$scope.userid+'/'+localStorage["custid"]+'/'+ordermonth
              , dataType: 'jsonp'
			}).then(function (response) {
              $scope.OrderssList = response.data;
			});
			...
			$scope.OrderListonDate = function(passedorderdate)
			{
				...
				$http({
              method: 'GET'
              , url: 'http://103.252.7.5:8086/api/OrderListonDate/'+localStorage["companyid"]+'/'+$scope.userlevel+'/'+$scope.userid+'/'+localStorage["custid"]+'/'+ordrdate
              , dataType: 'jsonp'
			}).then(function (response) {
              $scope.OrderssList = response.data;
			});
			...
			$scope.OrderPaymentListonMonth = function(month)
			{
				...
				$http({
              method: 'GET'
              , url: 'http://103.252.7.5:8086/api/OrderPaymentListonMonth/'+localStorage["companyid"]+'/'+$scope.userlevel+'/'+$scope.userid+'/'+localStorage["custid"]+'/'+ordermonth
              , dataType: 'jsonp'
			}).then(function (response) {
              $scope.OrderssList = response.data;
			});
			...
			$scope.OrderPaymentListonDate = function(passedorderdate)
			{
				...
				$http({
              method: 'GET'
              , url: 'http://103.252.7.5:8086/api/OrderPaymentListonDate/'+localStorage["companyid"]+'/'+$scope.userlevel+'/'+$scope.userid+'/'+localStorage["custid"]+'/'+ordrdate
              , dataType: 'jsonp'
			}).then(function (response) {
              $scope.OrderssList = response.data;
			});
			...
			$scope.GetVendorDetails = function(vendorid)
			{
				$http({
              method: 'GET'
              , url: 'http://103.252.7.5:8086/api/GetVendorDetails/'+vendorid
              , dataType: 'jsonp'
			}).then(function (response) {
             $scope.vendordetails = response.data;
				...
				$scope.SaveNewOrder= function() {
					...
					for(var i = 0 ; i < $scope.ProductList.length;i++)
					{
						if($scope.ProductList[i].qty)
						{
							...
							$scope.amount = parseFloat($scope.amount) + parseFloat($scope.orderdetails[0].rate)
						}
					}
					$scope.orderdetails[0].amount = $scope.amount;
				...
				$scope.UpdateOrder= function() {
					...
					else
					{
						...
						if($scope.ProductList[0].orderdate)
						{
				...
				$scope.getDashboardCount = function(interval,valuepassed)
			{
				...
				$http({
              method: 'GET'
              , url: 'http://103.252.7.5:8086/api/getDashboardCount/'+passinterval+'/'+interval+'/'+$scope.userlevel+'/'+localStorage["companyid"]+'/'+localStorage["custid"]
              , dataType: 'jsonp'
          }).then(function (response) {
			...
			$scope.getTotal = function(){
				var total = 0;
				for(var i = 0; i < $scope.ProductList.length; i++){
					if($scope.ProductList[i].qty == undefined || $scope.ProductList[i].qty == null)
						$scope.ProductList[i].qty = 0;
					total = parseInt(total) + parseInt($scope.ProductList[i].qty);
				}
				return total;
			}
		...
		$scope.setFocusTonext= function(prsevent,index)
		{
			if(prsevent.keyCode == '13')
			{	
